function Test_Destination_Node() {
	...
	if(!Destination_Blocked(_destination_x,_destination_y,_movement_type,_character,"Movement_Range")){
	    if(((_current_tile.running_cost + Get_Terrain_Stats(Get_Terrain_Type(_destination_x,_destination_y),"Movement_Cost",_movement_type))<=_movement_range)||(_character.control_in_battle && global.Infinite_Movement)){//if not outside move range
