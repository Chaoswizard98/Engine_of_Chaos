function Max_Magic_Boost_Battle_Cutscene_Event() {
	switch(state){
	    case "Hit_Animation":
	        ...
	        if(_target.on_screen){//if target is on screen
	            if(waiting_for_projectile){//if we're waiting for a projectile
	                ...
	                _message = Get_Character_Name("Local",target,"Total")+"'s maximum magic was boosted by " + string(_boost);
	                target.max_magic += _boost;
