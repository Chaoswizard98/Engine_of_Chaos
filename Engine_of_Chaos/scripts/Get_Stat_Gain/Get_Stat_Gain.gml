function Get_Stat_Gain() {
	var _lookup_type = argument[0];
	var _character = argument[1];
	var _return_stat = argument[2];
	var _current_stat = argument[3];
	var _set_average = argument[4];//do we force the stat to the set value for that level?
	var _catch_up = false;//do we correct shitty RNG this round?

	if(_return_stat = "New_Spell"){
	    return Get_Character_Level_Scheme_Stats(_lookup_type,_character,_return_stat,"New_Spell");
	}
	else if(_return_stat = "Increment_Spell_Level"){
	    return Get_Character_Level_Scheme_Stats(_lookup_type,_character,_return_stat,"Increment_Spell_Level");
	}
	else{
