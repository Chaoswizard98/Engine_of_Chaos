	...
	switch(state){
	    ...
	    case "Selected_Item":
	        if(Get_Inventory_Item_Stats(lookup_type,character_list[start_index + index],item_index,"Repairs_To") != "none"){//if we can repair it
	            ...
	            Create_Dialogue("none",Get_Voice(voice_ID),Get_General_Messages(obj_Shop_Controller.dialogue_ID,"Repair_For_X",string(floor(Get_Item_Stats(Get_Item(lookup_type,character_list[start_index + index],item_index),"Repair_Cost")*obj_Shop_Controller.repair_value_modifier))),id,false,false);//portrait, voice, message, current object, arrow, auto pan, [portrait override] [voice override]
            
	        }
	        else{//item isnt damaged
	            state = "Return_To_Shop";
	            Create_Dialogue("none",Get_Voice(voice_ID),Get_General_Messages(obj_Shop_Controller.dialogue_ID,"Not_Damaged"),id,true,true);//portrait, voice, message, current object, arrow, auto pan, [portrait override] [voice override]
	        }
	    break;
