function Spell_Stats_AU_Power_Wine(_effect_level,_lookup_type,_character){
	//================
	//Set Spell Stats=
	//================
	spell_name = Get_Spell_Display_Name("AU_Power_Wine","Spell_Name");//Get name from language script
	menu_name = Get_Spell_Display_Name("AU_Power_Wine","Menu_Name");//Get name from language script
	battle_cutscene_event = "AU_Power_Wine";
...
//=====================
function Town_Spell_Effect_State_Run_Effect_AU_Power_Wine(_object){
	with(_object){
		state = "End";
		var _boost = 2;
...
function AU_Power_Wine_Battle_Cutscene_Event() {
	switch(state){
	    case "Hit_Animation":
