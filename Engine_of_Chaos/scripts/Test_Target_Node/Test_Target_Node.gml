function Test_Target_Node() {
	...
	if(!Edge_Of_Room(_x_pos,_y_pos)){//If not outside room
	    var _new_tile = instance_position(_x_pos,_y_pos,obj_Target_Tile);
	    ...
	    if(_valid_tile){
	        if((_current_tile.running_cost + 1)<=_target_range){//if not beyond max range
	            if(_generate_new_tile){
	                _new_tile = instance_create_layer(_x_pos,_y_pos,"Terrain_Instance_Layer",obj_Target_Tile);//create new tile at location
	            }
