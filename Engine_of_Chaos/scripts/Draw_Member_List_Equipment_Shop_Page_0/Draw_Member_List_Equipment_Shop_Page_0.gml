function Draw_Member_List_Equipment_Shop_Page_0() {
	...
	draw_text(_class_title_x,_class_title_y,string_hash_to_newline(Get_Menu_Display_Message("Generic","ATTACK")));//CLASS
	draw_text(_level_title_x,_level_title_y,string_hash_to_newline(Get_Menu_Display_Message("Generic","DEFENSE")));//LEVEL
	...
	for (i = start_index; i < min((start_index + max_characters_displayed),number_of_characters); i+= 1){
	    ...
	    while(j<global.Number_Of_Equipment_Slots){//loop through equipment, stop on first slot the item can be equipped to
	        if(Get_Item_Stats(obj_Shop_Controller.item_array[(obj_Shop_Controller.page_number*obj_Shop_Controller.items_per_page)+obj_Shop_Controller.item_index],Get_Equipment_Slot_Name(j),lookup_type,character_list[i])){//if character can equip item to this slot
	            draw_text(_class_field_x,_class_field_y+ y_scroll + (character_index_offset_y * (i - start_index)),string_hash_to_newline(Get_Equipped_Item_Stats(lookup_type,character_list[i],j,"Attack")));
	            draw_sprite(spr_Equip_Arrow,0,_class_field_x + 8,_class_field_y+ y_scroll + (character_index_offset_y * (i - start_index)));
	            draw_text(_class_field_x+16,_class_field_y+ y_scroll + (character_index_offset_y * (i - start_index)),string_hash_to_newline(string(Get_Item_Stats(obj_Shop_Controller.item_array[(obj_Shop_Controller.page_number*obj_Shop_Controller.items_per_page)+obj_Shop_Controller.item_index],"Attack",lookup_type,character_list[i]))));
	            break;
	        }
	        j += 1;
	    }
	    if(j >= global.Number_Of_Equipment_Slots){//if we cant equip this item, 
	        draw_text(_class_field_x,_class_field_y+ y_scroll + (character_index_offset_y * (i - start_index)),string_hash_to_newline(Get_Menu_Display_Message("Generic","unequippable")));
	    }
	}
