function Test_AI_Movement_Node() {
	...
	if(!Destination_Blocked(_destination_x,_destination_y,_movement_type,_character,"AI_Pathfinding")){
	    if(((_current_tile.running_cost + Get_Movement_Cost(Get_Terrain_Type(_destination_x,_destination_y),_movement_type))<=_movement_range)){//if not outside move range
