function Draw_Control_Config_Menu() {
	//===============
	//Draw Locations=
	//===============
	...
	var _config_text_x = _text_x + 116;
	...
	var _scroll_arrow_x = _text_x + 228;
	var _scroll_arrow_y = _text_y;
	...
	if(state != "Mapping"){
	    ...
	    draw_sprite(spr_Menu_Arrow_Right,0,_config_arrow_x, _config_arrow_y + (arrow_position * _text_increment));//Draw the selection arrow
	}
	...
	for(i = start_index; i < (start_index + max_options_on_screen); i += 1){
	    ...
	    draw_text(_config_text_x, _config_text_y + ((i - start_index) * _text_increment), string_hash_to_newline(Get_Control_Names(settings_array[i]))); //draw the message
	}
